...
class TakeKeyCommand extends BaseCommand{
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args) : bool{
        ...
        if($args[1] == in_array($args[1], ["Common", "Vote", "Rare", "Legendary"])){
            ...
            $this->cfg->save();
            $sender->sendMessage(C::GREEN . "Successfully Removed {$player->getName()} $args[2] $args[1] Crate Key!");
            $player->sendMessage(C::RED . "$args[2] $args[1] Crate key has been removed from your account!");
            $player->sendMessage(C::RED . "You now have " . $this->cfg->get($args[1]) . " $args[1] Key.");
        }else{
